...
function Profil($bdd, $user_name, $password, $confirm_password){
  ...
  function ajout_msg($bdd, $messages){
    $error = null;
    $messages = mysqli_escape_string($bdd, htmlspecialchars(trim($messages)));
    $mon_id = $_SESSION['id'];

    if (!empty($messages)) {

        $insert = mysqli_query($bdd, "INSERT INTO messages(message, id_utilisateur, date) VALUES('$messages', '$mon_id', NOW())");
        if ($insert) {
      ...
      $error = "Vous n'avez pas rentré de messages";
    }
    return $error;
  }
